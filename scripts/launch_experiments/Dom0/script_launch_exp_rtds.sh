...
for((j=0;j<4;j++)) do
        ...
        ## Only if the sched is RTDS
        xl sched-rtds -d guest    -v all -p ${period[j]} -b ${budget[j]} -e 1
        xl sched-rtds -d guest1   -v all -p ${period[j]} -b ${budget[j]} -e 1
        xl sched-rtds -d Domain-0 -v all -p ${period[j]} -b ${budget[j]} -e 1
        xl sched-rtds -v all
		...
		pid_voter=$!
                ...
                ssh -i /etc/dropbear/id_rsa root@$IP_A sh /root/script_A.sh $IP_                                                                                                             DOM0 $PORT_A &
                ssh -i /etc/dropbear/id_rsa root@$IP_B sh /root/script_B.sh $IP_                                                                                                             DOM0 $PORT_B &
